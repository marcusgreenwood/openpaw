...
- **Build:** `npm run build`
- **Lint:** `npm run lint` (ESLint; expected to pass cleanly — 0 errors, 0 warnings, exit code 0)
...
- The lint command (`npm run lint`) is expected to exit 0 with no errors and no warnings. A non-zero exit means the current change introduced a problem — it is not a known pre-existing issue, so fix it rather than ignoring it.
