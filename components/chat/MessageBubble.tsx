...
function renderToolPart(part: any, key: string, onChoiceSelect?: (option: string) => void) {
  const { type, state, input, output } = part;

  // Extract tool name from "tool-xxx" type
  ...
  // Still executing (input-streaming or input-available)
...
export function MessageBubble({ message, onChoiceSelect }: MessageBubbleProps) {
  ...
  return (
    <div className={cn("flex mb-4", isUser ? "justify-end" : "justify-start")}>
      <div
        className={cn(
          "max-w-3xl max-[768px]:max-w-[85vw]",
